...
global nodes
    ...
    for x = 1:length(data)
        ...
        try
            check = any(contains(data{x}.fullname,{'Spec Prob','Spec Topics', 'Special Topics', 'Special Problems', 'Undergrad', 'Graduate', 'Research', 'Seminar'}));
        catch
            check = any(myContains(data{x}.fullname,{'Spec Prob', 'Spec Topics','Special Topics', 'Special Problems', 'Undergrad', 'Graduate', 'Research', 'Seminar'}));
        end
        if  check ...
...
disp('Making groups...')
    ...
    for pos = 1:length(groups)
        if sum(colors(pos, :)) < 100
            colors(pos,:) = colors(pos, :) + 100;
        end
        myColor = sprintf('rgba(%d,%d,%d,1)', colors(pos,1), colors(pos,2), colors(pos,3));
        str = [str, sprintf('%s: {"color": "%s"},\n', groups{pos}, myColor)];
    end
    str(end-1) = [];
    str = [str, '};'];
    fprintf(fh,str);
